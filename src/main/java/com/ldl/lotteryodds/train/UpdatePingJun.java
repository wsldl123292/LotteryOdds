 ...
 * 时间: 2015/9/28 17:44
 */
public class UpdatePingJun {

    public static void main(String[] args) {

        //采集开始时间2011-07-21

        /*LocalDate beginDate = LocalDate.of(2012, 7, 3);
        final LocalDate endDate = LocalDate.of(2011, 12, 31);*/

        LocalDate beginDate = LocalDate.of(2013, 4, 27);
        final LocalDate endDate = LocalDate.of(2012, 12, 31);

        /*LocalDate beginDate = LocalDate.of(2015, 9, 23);
        ...
        while (beginDate.isAfter(endDate)) {
            ...
            try {
                ...
                for (Object tr : trs) {
                    ...
                    //获取欧赔数据表格
                    final Element opDatatb = oupei.getElementById("table_btm").select("table").first();
                    final Element optr = opDatatb.select("tbody>tr").select("[xls=footer]").first();
                    ...
                    oddInfo.setScwOdd(cop.get(0).text());
                    oddInfo.setScdOdd(cop.get(1).text());
                    oddInfo.setSclOdd(cop.get(2).text());

                    //最终赔率
                    final Elements lop = cpktrs.get(1).select("td");
                    oddInfo.setSlwOdd(lop.get(0).text().replace("↑", "").replace("↓", ""));
                    oddInfo.setSldOdd(lop.get(1).text().replace("↑", "").replace("↓", ""));
                    oddInfo.setSllOdd(lop.get(2).text().replace("↑", "").replace("↓", ""));
                    ...
                    oddInfo.setScwKl(ckl.get(0).text());
                    oddInfo.setScdKl(ckl.get(1).text());
                    oddInfo.setSclKl(ckl.get(2).text());

                    //最终凯利指数
                    final Elements lkl = klktrs.get(1).select("td");
                    oddInfo.setSlwKl(lkl.get(0).text().replace("↑", "").replace("↓", ""));
                    oddInfo.setSldKl(lkl.get(1).text().replace("↑", "").replace("↓", ""));
                    oddInfo.setSllKl(lkl.get(2).text().replace("↑", "").replace("↓", ""));
                    oddInfos.add(oddInfo);
                }


                if (count == 5) {
                    final int size = lotteryOddsDao.batchUpdateOddInfoAvg(oddInfos);
                    oddInfos.clear();
                    count = 0;
