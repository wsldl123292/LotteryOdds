...
public class OddsCrawling {
    ...
    public static void main(String[] args) {
        ...
        final LocalDate endDate = LocalDate.of(2015, 7, 23);
        int count = 0;
        final CloseableHttpClient client = HttpClientBuilder.create().build();
        CloseableHttpResponse response;
        final String baseurl = "http://live.500.com/?e=";
        final List<Entity500> entity500s = new ArrayList<>();
        String url;
        ...
        while (beginDate.isAfter(endDate)) {
            ...
            try {
                ...
                final Map<String, Object> liveOddsMap = (Map) JSON.parse(odds.substring("var liveOddsList=".length(), odds.length() - 1));
                final Map<String, Map<String, String>> oddMaps = new HashMap<>();
                for (String key : liveOddsMap.keySet()) {
                    final String value = liveOddsMap.get(key).toString().replace("\"", "").replace("[", "").replace("],", "-").replace("{", "").replace("}", "").replace("]", "");
                ...
                for (Object tr : trs) {
                    ...
                    final Entity500 entity500 = new Entity500();
                    entity500.setMatchid(fid);
                    final Map<String, String> map = oddMaps.get(fid);
                    //拼接赔率
                    String rqhtml = "";
                    if (map.get("sp") != null) {
                        final String[] arr = map.get("sp").split(",");
                    ...
                    if (map.get("rqsp") != null) {
                        final String[] arr = map.get("rqsp").split(",");
